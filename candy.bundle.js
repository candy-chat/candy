...
Candy.Core.Action = (function(self, Strophe, $) {
	...
	self.Jabber = {
		...
		ResetIgnoreList: function() {
			Candy.Core.getConnection().send($iq({type: 'set', from: Candy.Core.getUser().getJid().replace(' ', '\\20').replace(' ','\\20'), id: 'set1'})
		...
		GetIgnoreList: function() {
			Candy.Core.getConnection().send($iq({type: 'get', from: Candy.Core.getUser().getJid().replace(' ', '\\20').replace(' ','\\20'), id: 'get1'})
				.c('query', {xmlns: Strophe.NS.PRIVACY }).c('list', {name: 'ignore'}).tree());
		},
		...
		SetIgnoreListActive: function() {
			Candy.Core.getConnection().send($iq({type: 'set', from: Candy.Core.getUser().getJid().replace(' ', '\\20').replace(' ','\\20'), id: 'set2'})
		...
		Room: {
			...
			Disco: function(roomJid) {
				Candy.Core.getConnection().send($iq({type: 'get', from: Candy.Core.getUser().getJid().replace(' ','\\20').replace(' ','\\20'), to: roomJid, id: 'disco3'}).c('query', {xmlns: Strophe.NS.DISCO_INFO}).tree());
			},
