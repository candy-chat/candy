...
Candy.View.Pane.Roster = function(self, parent) {
    var _batchRosterUpdateTimeout, _batchRosterUpdateList = {}, _batchRosterUpdateCallback = function _batchRosterUpdateCallback() {
        var newTimeout = false, batchRosterOpts = Candy.View.getOption("bigroom").batchRosterUpdate, threshold = batchRosterOpts.threshold;
        ...
        if (newTimeout) {
            _batchRosterUpdateTimeout = setTimeout(_batchRosterUpdateCallback, batchRosterOpts.interval);
        } else {
    ...
    var join = function join(roomId, roomJid, userId, user, userElem, currentUser) {
        var usercountDiff = 1, html = Mustache.to_html(Candy.View.Template.Roster.user, {
            roomId: roomId,
            userId: userId,
            userJid: user.getJid(),
            nick: user.getNick(),
        ...
        if (userElem.length < 1) {
            var userInserted = false, rosterPane = parent.Room.getPane(roomJid, ".roster-pane"), userCount = rosterPane.children().length, bigroomOpts = Candy.View.getOption("bigroom"), disableSortingThreshold = bigroomOpts.disableSortingThreshold, batchRosterUpdateThreshold = bigroomOpts.batchRosterUpdate.threshold;
    ...
    self.update = function update(roomJid, user, action, currentUser) {
        ...
        var roomId = parent.Chat.rooms[roomJid].id, userId = Candy.Util.jidToId(user.getJid()), usercountDiff = -1, userElem = $("#user-" + roomId + "-" + userId);
    ...
    self.joinAnimation = function joinAnimation(elementId) {
        var roomJid = Candy.View.getCurrent().roomJid, roomUserCount = Candy.View.Pane.Chat.rooms[roomJid].usercount;
