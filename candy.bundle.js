    ...
    self.Jabber = {
        ...
        Room: {
            ...
            Message: function(roomJid, message, type, xhtmlMessage) {
                ...
                roomJid = Candy.Util.escapeJid(roomJid);
                var conn = Candy.Core.getConnection(), msg = $msg({
                    to: roomJid,
                    from: conn.jid,
                    type: type,
            ...
            Admin: {
                ...
                SetSubject: function(roomJid, subject) {
                    var conn = Candy.Core.getConnection(), id = "sbjc:" + conn.getUniqueId();
                    conn.send($msg({
                        to: Candy.Util.escapeJid(roomJid),
                        id: id,
                        from: conn.jid,
    ...
    self.Jabber = {
        ...
        Presence: function(msg) {
            ...
            if (msg.children('x[xmlns^="' + Strophe.NS.MUC + '"]').length > 0) {
				 ...
				 */
                $(Candy).triggerHandler("candy:core.presence.update", {
                    from: msg.attr("from"),
                    stanza: msg
                });
            }
        ...
        Room: {
            ...
            PresenceDispatcher: function($stanza) {
                ...
                var from = $stanza.attr("from"), roomJid = Strophe.getBareJidFromJid(from), presenceType = $stanza.attr("type"), isJoin = presenceType !== "unavailable", isLeave = !isJoin, statuses = Candy.Util.parseStatusCodes($stanza), isMe = statuses[110] !== undefined, isNickChange = statuses[303] !== undefined, isNickAssign = statuses[210] !== undefined, isKick = statuses[307] !== undefined, isBan = statuses[301] !== undefined, item = $stanza.find("item"), user, action;
            ...
            PresenceError: function($stanza) {
                ...
                var from = $stanza.attr("from"), roomJid = Strophe.getBareJidFromJid(from), room = Candy.Core.getRooms()[roomJid], roomName = room.getName();
...
Candy.View.Pane = function(self, $) {
    ...
    self.Message = {
        ...
        submit: function(event) {
            var roomJid = Candy.View.getCurrent().roomJid, roomType = Candy.View.Pane.Chat.rooms[roomJid].type, message = $(this).children(".field").val().substring(0, Candy.View.getOptions().crop.message.body);
            var evtData = {
                roomJid: roomJid,
                message: message
            };
...
/* global Candy */
/** Variable: Candy.View.Translation.ca
 * Catalan translation
 */
Candy.View.Translation.ca = {
    status: "Estat: %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.es
 * Spanish translation
 */
Candy.View.Translation.es = {
    status: "Estado: %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.fr
 * French translation
 */
Candy.View.Translation.fr = {
    status: "Status : %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.it
 * Italian translation
 */
Candy.View.Translation.it = {
    status: "Stato: %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.ja
 * Japanese translation
 */
Candy.View.Translation.ja = {
    status: "ステータス: %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.nl
 * Dutch translation
 */
Candy.View.Translation.nl = {
    status: "Status: %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.pt
 * Portuguese translation
 */
Candy.View.Translation.pt = {
    status: "Status: %s",
    statusConnecting: "Conectando...",
    statusConnected: "Conectado",
...
/* global Candy */
/** Variable: Candy.View.Translation.pt_br
 * Brazilian Portuguese translation
 */
Candy.View.Translation.pt_br = {
    status: "Estado: %s",
    statusConnecting: "Conectando...",
    statusConnected: "Conectado",
    statusDisconnecting: "Desconectando...",
...
/* global Candy */
/** Variable: Candy.View.Translation.ru
 * Russian translation
 */
Candy.View.Translation.ru = {
    status: "Статус: %s",
...
/* global Candy */
/** Variable: Candy.View.Translation.sv
 * Swedish translation
 */
Candy.View.Translation.sv = {
    status: "Status: %s",
